...
Since AlphaGo's defeat of the Go champion Ke Jie, the artificial intelligence community has
...
\begin{align}
  \bm{V} = \mathcal{N}(\bm{I})
\end{align}
We choose the next action according to the equation:
\begin{align}
...
The learning environment was based on the existing mini game environments from the PySC2 API. Specifically, our learning environment was built on top of the DefeatRoaches mini game.

The maps were retrieved from Deepmind's PySC2 Github repository and 
put into the StarcraftII/Maps/ directory.

All maps were edited in the SCII map editor. 

\subsubsection{Learning Environment}
The DefeatRoaches mini game is 64x64 simple map without any obstacles that spawns a group of marines and roaches at two spawn points within the map. 

\subsubsection{Map Variations}
DefeatRoaches 1v1 - 
Similar to the DefeatRoaches map except for the number of marines and 
...
DefeatRoaches96 - 
This variation, DefeatRoaches96, is a 96x96 map that is bigger than the original. This allows the agent to have a larger input space to choose from which could potentially improve rate of convergence. 
